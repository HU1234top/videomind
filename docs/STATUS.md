...
| Knowledge Builder | `src/builders/knowledge-builder.mjs` | 8-category classification, Levenshtein title dedup (0.6 threshold) |
...
| CLI | `src/cli.mjs` | collect/analyze/build/sync commands, rate limiter stats on completion |
...
| 5. **Adaptive rate limiting** | **✅ DONE** | **See `src/core/rate-limiter.mjs` — 29 tests passing** |
...
| Obsidian Sink | ⚠️ Partial | Markdown output has wikilinks, but no .obsidian config |
